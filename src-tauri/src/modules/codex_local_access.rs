...
const CODEX_PROVIDER_GATEWAY_MODEL_SLOTS: [&str; 3] = ["gpt-5.5", "gpt-5.4", "gpt-5.4-mini"];
...
const DEFAULT_MAX_RETRY_INTERVAL_MS: u64 = 30 * 1000;
...
fn pin_account_to_front_for_strategy(
    ...
    // Existing affinity always wins. Backup classification only controls the
    // first account chosen for a new session.
    if preferred_account_id.is_some() {
...
async fn prepare_sidecar_launch_config_in_dir(
    ...
    for account_id in effective_sidecar_account_ids(collection) {
        ...
        let (account, is_override_account) =
...
fn collection_tracks_sidecar_quota_snapshot(
    collection: &CodexLocalAccessCollection,
    account_id: &str,
) -> bool {
    let account_id = account_id.trim();
    ...
    let service_member_matches = effective_sidecar_account_ids(collection)
        .iter()
    ...
    let bound_reserve_matches = collection.bound_oauth_quota_reserve.is_some()
        && normalize_optional_account_ref(collection.bound_oauth_account_id.as_deref())
            .as_deref()
            == Some(account_id);
    service_member_matches || bound_reserve_matches
}
...
pub async fn reevaluate_bound_oauth_quota_reserve_after_refresh(
    ...
    let matching_collection = {
        ...
        let collection = runtime
            .collection
            .as_ref()
            .filter(|collection| collection_tracks_sidecar_quota_snapshot(collection, account_id))
            .cloned();
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct ProviderGatewayModelSlot {
    client_model: String,
...
fn provider_gateway_model_slots(models: &[String]) -> Vec<ProviderGatewayModelSlot> {
    normalize_provider_gateway_models(models.iter().map(String::as_str).collect())
        .into_iter()
        .zip(CODEX_PROVIDER_GATEWAY_MODEL_SLOTS)
        .map(|(upstream_model, client_model)| ProviderGatewayModelSlot {
...
pub async fn activate_provider_gateway_for_dir(
    ...
    if let Some(default_slot) = model_slots.first() {
        write_local_access_profile_model_override(profile_dir, &default_slot.client_model)?;
    }
    if !model_slots.is_empty() {
        write_provider_gateway_model_catalog(profile_dir, &model_slots)?;
    }
    ensure_runtime_loaded_without_start().await?;
    let runtime = gateway_runtime().lock().await;
    Ok(build_state_snapshot(&runtime))
...
mod tests {
    ...
    use super::{
        ...
        is_responses_completion_event, is_sidecar_quota_reserve_write_error,
        ...
        normalize_collection_api_keys, normalize_custom_routing_rules,
        ...
        prepare_websocket_initial_request, prioritize_account_pool, profile_base_url_matches,
        ...
        provider_gateway_models_for_account, read_http_request,
        reconcile_gateway_result_last_error, recover_invalid_stats_file,
        ...
        resolve_effective_model_pricing, resolve_plan_rank,
        ...
        system_proxy_target_scheme, system_proxy_value_url, validate_bound_oauth_quota_reserve,
    ...
    #[test]
    fn sidecar_quota_hot_refresh_matches_every_service_member() {
        let mut collection = test_local_access_collection(vec!["plus-account".to_string()]);
        assert!(super::collection_tracks_sidecar_quota_snapshot(
            &collection,
            "plus-account"
        ));
        assert!(!super::collection_tracks_sidecar_quota_snapshot(
        ...
        collection.account_ids.clear();
        collection.bound_oauth_account_id = Some("reserved-account".to_string());
    ...
    #[test]
    fn legacy_custom_collection_with_nullable_weekly_reserve_keeps_api_key_and_fields() {
        let value = json!({
            "enabled": true,
            "port": 63685,
            "apiKey": "stable-existing-key",
            "routingStrategy": "custom",
            "customRoutingRules": [{
                "accountId": "k12-account",
                "priority": 80,
                "weight": 2
            }],
            "sessionAffinity": true,
            "newSessionPriorityEnabled": true,
            "newSessionPriorityAccountIds": ["k12-account"],
            "boundOauthAccountId": "plus-account",
            "boundOauthQuotaReserve": {
                "hourlyPercent": 10,
                "weeklyPercent": null
            },
            "accountIds": ["k12-account", "plus-account"],
            "createdAt": 1,
            "updatedAt": 2
        });

        let mut collection: CodexLocalAccessCollection =
            serde_json::from_value(value).expect("legacy custom collection should deserialize");

        assert_eq!(collection.api_key, "stable-existing-key");
        assert_eq!(
            collection
                .bound_oauth_quota_reserve
                .as_ref()
                .and_then(|reserve| reserve.weekly_percent),
            None
        );
        assert_eq!(
            collection.new_session_priority_account_ids,
        ...
        assert!(!collection.custom_routing_rules[0].is_backup);

        assert!(normalize_collection_api_keys(&mut collection));
        assert_eq!(collection.api_key, "stable-existing-key");
        assert_eq!(collection.api_keys[0].key, "stable-existing-key");
    }
...
"#,
    ...
    #[test]
    fn custom_routing_keeps_backup_accounts_for_new_sessions_but_preserves_affinity() {
        ...
        let prioritized = prioritize_account_pool(ordered.clone(), &["backup".to_string()]);
        let new_session_ordered = pin_account_to_front_for_strategy(
            prioritized,
            None,
            CodexLocalAccessRoutingStrategy::Custom,
            &rules,
        ...
        assert_eq!(new_session_ordered, vec!["regular", "backup"]);
        assert_eq!(affinity_ordered, vec!["backup", "regular"]);
    }
...
"#;
    ...
    #[tokio::test]
    async fn local_access_takeover_uses_dynamic_model_discovery() {
    ...
    #[test]
    fn sidecar_codex_key_skips_loopback_base_url_without_provider_recovery() {
        let account = CodexAccount::new_api_key(
            "api-loopback-1".to_string(),
            "api-key@example.com".to_string(),
            "sk-test".to_string(),
            CodexApiProviderMode::Custom,
            Some("http://localhost:53549/v1".to_string()),
