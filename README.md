...
- **同一会话并行一致性**：同一个真实会话的并行请求不会被拆到不同账号；需要切号时会释放失败尝试的临时占用，并等待其他并行请求结束或汇合到同一个目标账号。等待可被客户端取消，也不会占住全局路由锁。
...
- **流式请求至多发送一次**：首响应默认等待 180 秒。`/v1/responses` 请求体一旦被 HTTP transport 读取，首包超时、客户端取消、EOF、空流或 5xx 都不会自动重发、换模型或换账号；只有 DNS、TCP、TLS 等明确发生在请求体零字节发送前的失败才允许按配置尝试其他账号。客户端显式提供的 `Idempotency-Key` 会原样转发；未提供时使用稳定的 `logical_request_id`，使支持幂等键的中转站能够识别同一逻辑请求。
- **可核对的请求账目**：每个客户端逻辑请求都有稳定 `logical_request_id`，每个真实上游 POST 都有独立 `upstream_attempt_id`。Cockpit 分开统计逻辑请求、上游实际尝试、成功、取消和“可能已计费但无法取得 usage”的请求；取消后迟到的完整 usage 会回写原请求记录，不再静默丢失。
