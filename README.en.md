...
- **At-most-once streaming requests**: The first response now waits 180 seconds by default. Once the HTTP transport reads any `/v1/responses` request-body bytes, a first-payload timeout, client cancellation, EOF, empty stream, or 5xx response cannot resend the request, switch models, or rotate accounts. Only a confirmed zero-byte pre-send DNS, TCP, or TLS failure may try another account. An explicit client `Idempotency-Key` is forwarded unchanged; otherwise the stable `logical_request_id` is used so gateways that support idempotency can recognize the same logical request.
- **Reconciled request accounting**: Every client request has a stable `logical_request_id`, and every physical upstream POST has its own `upstream_attempt_id`. Cockpit reports logical requests, actual upstream attempts, successes, cancellations, and possibly billable attempts without usage separately. Complete usage that arrives after cancellation updates the original request record instead of being silently dropped.
