...
- **K12 优先并允许单账号溢出**：有两个及以上 K12 可承接新会话时仍只在 K12 间均衡；仅剩一个可用 K12 且存在并发压力时，新会话按约 **2:1** 分配给该 K12 与合格的非 K12 / Plus，避免所有并发集中到最后一个 K12。已确认的 K12 旧会话不会因此迁移。
...
- 通过 API 服务绑定的 OAuth 账号通常作为最终兜底；仅在只剩一个 K12 可承接新会话且其近期负载较高时，才会提前分担约三分之一的新会话。
