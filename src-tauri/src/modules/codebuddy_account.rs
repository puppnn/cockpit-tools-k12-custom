...
fn save_account_file(account: &CodebuddyAccount) -> Result<(), String> {
    ...
    let content =
        crate::modules::secure_account_storage::serialize_account_file("codebuddy", account)?;
...
pub fn list_accounts() -> Vec<CodebuddyAccount> {
    ...
    if had_index_accounts && accounts.is_empty() {
        logger::log_warn(
            "[CodeBuddy Account] 账号索引中存在账号，但详情文件均无法读取，已跳过空索引写回",
        );
        return accounts;
    }
    if let Err(err) = save_account_index(&index) {
...
pub fn list_accounts_checked() -> Result<Vec<CodebuddyAccount>, String> {
    ...
    let accounts = normalize_account_index(&mut index);
    if had_index_accounts && accounts.is_empty() {
        return Err("CodeBuddy 账号索引中存在账号，但详情文件均无法读取；已保留前端缓存，请从账号备份或本地账号文件恢复。".to_string());
    }
    if let Err(err) = save_account_index(&index) {
        logger::log_warn(&format!("[CodeBuddy Account] 保存账号索引失败: {}", err));
    }
...
pub fn sync_account_to_default_client(account_id: &str) -> Result<(), String> {
    let account =
        load_account(account_id).ok_or_else(|| format!("CodeBuddy 账号不存在: {}", account_id))?;
    let state_db = get_default_codebuddy_state_db_path()
        .ok_or_else(|| "无法定位默认 CodeBuddy state.vscdb 路径".to_string())?;
    if !state_db.exists() {
        return Err(format!(
            "未找到默认 CodeBuddy state.vscdb: {}",
            state_db.display()
        ));
    }
